...
\begin{center}
\begin{tikzpicture}[scale=1.6, transform shape]
\begin{axis}[xmin=-0.2, xmax=3, ymin=-2.2, ymax=10, axis lines=middle,
  xlabel=$x$,ylabel=$y$,
  xtick={0.2,1,1.8,2.8},
  xticklabels={$c$, $e$, $d$, $f$},
  xticklabel style={anchor=north, font=\footnotesize},
  ytick={5.8064,5.1449,7.8125,8.8064,2.8064},
  yticklabels={$g(s)$, $g(p)$, $g(q)$,$g(s)+\frac{\varepsilon }{2}$, $g(s)-\frac{\varepsilon }{2}$},
  every major tick/.append style={thick, major tick length=5pt, black},
  yticklabel style={anchor=east, font=\footnotesize}
  ]
  \addplot[color=red, domain=0:3, samples=100, thick]{-(x-1)^4 + (x-1)^3 + 2*(x-1)^2+5} node[above=5pt, pos=0.7] {$g$};
  \filldraw[black] (axis cs:0.7,0) circle (2pt) node[below=3pt, font=\footnotesize]{$p$};
  \filldraw[black] (axis cs:1.6,0) circle (2pt) node[below=3pt, font=\footnotesize]{$s$};
  \filldraw[black] (axis cs:2.5,0) circle (2pt) node[below=3pt, font=\footnotesize]{$q$};
  \draw[->] (axis cs:0.7,-1.3) -- (axis cs:2.5,-1.3);
  \draw (axis cs:1.6,-1.3) node[below, font=\footnotesize] {$\abs{p-q} < \delta $};
  \draw[->] (axis cs:2.5,-1.3) -- (axis cs:0.7,-1.3);
  \draw[color=blue, dashed] (axis cs:1.6,0) -- (axis cs:1.6,5.8064);
  \draw[color=blue, dashed] (axis cs:0,5.8064) -- (axis cs:1.6,5.8064);
  \draw[color=gray, dashed] (axis cs:0.7,0) -- (axis cs:0.7,5.1449);
  \draw[color=gray, dashed] (axis cs:0,5.1449) -- (axis cs:0.7,5.1449);
  \draw[color=gray, dashed] (axis cs:2.5,0) -- (axis cs:2.5,7.8125);
  \draw[color=gray, dashed] (axis cs:0,7.8125) -- (axis cs:2.5,7.8125);
\end{axis}
\end{tikzpicture}
\end{center}
  \caption{Første del af beviset af sætning \ref{theo:ulighed_overundersum}}%
